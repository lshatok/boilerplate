...
TELEGRAF_SQL_FILE=''
# -- reporting -- #
IMFLUX_DB_NAME=reporting_db
IMFLUX_DB_USER=reporting
IMFLUX_DB_PASS='influx108'
IMFLUX_SQL_FILE=''
...
POSTGRES_PORT=3306
POSTGRES_ADMIN_USER={{ product.admin }}
...
choose_environment() {
    ...
    while [ $PASSED == "no" ]; do
        ...
        case ${POSTGRES_ENV_NAME} in
            dev|dev[0-9][0-9]|test|test[0-9][0-9]|pilot|pilot[0-9][0-9]|demo|demo[0-9][0-9]|prod|prod[0-9][0-9])
                PASSED="yes"
...
choose_apps() {
    echo
    APPS_LIST="grafana grafana snmp telemetrix fluxe telegraf reporting DONE"
    webtelemetry=''
    grafana=''
    snmp=''
    telemetrix=''
...
EOF
        ...
        case $opt in
            ...
            "7")    if [[ ${reporting} ]]; then
                        ...
                        [[ ${POSTGRES_PROD} == "yes" ]] && IMFLUX_DB_PASS=$(randpw)
                        read -p "Enter the REPORTING user DB password [${IMFLUX_DB_PASS}]: " pass_w
                        [[ ${pass_w} ]] && IMFLUX_DB_PASS=${pass_w}
                        unset pass_w
                        read -p "Enter SQL file to load [none]: " IMFLUX_SQL_FILE
                        reporting='X'
                    fi
                ;;
...
review_settings() {
    ...
    if [[ ${reporting} ]]; then
        echo "REPORTING --"
        echo "   db name:  ${IMFLUX_DB_NAME}"
        echo "   db user:  ${IMFLUX_DB_USER}"
        echo "   db pass:  ${IMFLUX_DB_PASS}"
        [[ ${IMFLUX_SQL_FILE} ]] && echo "  sql file:  ${IMFLUX_SQL_FILE}"
        echo
    fi
...
create_databases() {
    ...
    if [[ ${reporting} ]]; then
        echo "  reporting"
        _create_db ${IMFLUX_DB_USER} ${IMFLUX_DB_PASS} ${IMFLUX_DB_NAME} ${IMFLUX_SQL_FILE}
    fi
}
